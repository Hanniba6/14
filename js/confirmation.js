...
function loadOrderDetails123() {
    ...
    tempOrder.items.forEach(item => {
        ...
        orderItem.innerHTML = `
            <div class="order-item-details">
                <p class="order-item-title">${resource.name}</p>
                <p class="order-item-price">$${resource.price.toFixed(2)} × ${item.quantity}</p>
            </div>
            <div class="order-item-quantity">
                <p>$${(resource.price * item.quantity).toFixed(2)}</p>
            </div>
        `;
    ...
    tempOrder.subtotal = tempOrder.total;
    localStorage.setItem('tempOrder', JSON.stringify(tempOrder));
    
    // Display total
    orderTotalAmount.textContent = `$${tempOrder.total.toFixed(2)}`;
